<?php
	...
	class User {
		...
		/**
		 * public userWithNameExists($_username)
		 *
		 * Returns true if a user exists with the given username
		 * in the users or users_confirm table.
		 */
		public function userWithNameExists($username) {
			$db = Database::getInstance();
			$query = "SELECT username FROM users WHERE username = ? UNION SELECT username FROM users_confirm WHERE username = ?";
		...
		/**
		 * public userWithEmailExists($_username)
		 *
		...
		public function userWithEmailExists($email) {
			$db = Database::getInstance();
			$query = "SELECT email FROM users WHERE email=? UNION SELECT email FROM users_confirm WHERE email=?";
			$db->query($query, array($email, $email));
			$result = $db->count();
			return $result;
		}
		...
		public function queueNewUser($email, $username, $pw) {
			...
			// Send email
			$em = new Email();
			$em->sendEmail($email, "Confirm your account", "You have recently registered for a GHOSTx68000 account. Please click here to confirm your registration: ".Config::get('domain').Config::get('directory')."verify.php?key=".$key, true);
